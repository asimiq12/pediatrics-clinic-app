...
if appt_type == "Checkup":
    doctor_list = shift_data.get(date_str, [])
    if doctor_list:
        doctor = st.selectbox("Choose Doctor", doctor_list)
    else:
        st.warning("No doctors assigned for this day.")
        st.stop()
else:
    # For Sick/Consultation, assign doctor automatically (first available)
    doctor_list = shift_data.get(date_str, [])
    if not doctor_list:
        st.warning("No doctors assigned for this day.")
        st.stop()
    doctor = doctor_list[0]  # auto-select first doctor
...
if appt_date.weekday() == 5:
    if appt_type == "Checkup":
        ...
        if appt_date != today:
            st.warning("Saturday sick visits can only be booked on the same day.")
            st.stop()
